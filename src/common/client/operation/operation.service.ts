...
export class OperationService {
  private logger = new Logger(`::${CLIENT.OPERATION}::${OperationService.name}`);
  constructor(
  ...
  callFxSearchCourseTeacher<TResult = any, TInput = any>(
    ...
    const pattern = { subjet: 'client-operation', function: 'search-course-teacher' };
  ...
  callFxSearchQualificationTeacher<TResult = any, TInput = any>(
    ...
    const pattern = { subjet: 'client-operation', function: 'search-qualification-teacher' };
    return this.client.send<TResult, TInput>(pattern, dto).toPromise();
  }
